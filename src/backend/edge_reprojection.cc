...
//    std::cout << pts_i_.transpose() <<" "<<pts_j_.transpose()  <<std::endl;
    ...
    // === 观测/状态合法性检查 ===
    // 逆深度必须为有限值且为正(对应深度 > 0)；否则跳过该 edge,残差置零
    if (!std::isfinite(inv_dep_i) || inv_dep_i < 1e-3) {
        std::cerr << "[EdgeReprojection] WARN: invalid inv_dep_i=" << inv_dep_i
                  << " (vertex id=" << verticies_[0]->Id() << "), skip residual\n";
        residual_.setZero();
        return;
    }
    ...
    if (!Pi.allFinite() || !Pj.allFinite() || !tic.allFinite()) {
        std::cerr << "[EdgeReprojection] WARN: pose params has NaN/Inf, skip residual\n";
        residual_.setZero();
        return;
    }
    ...
    // 投影深度异常 (z<=0 或 NaN) 直接判废
    if (!std::isfinite(dep_j) || dep_j < 1e-3) {
        std::cerr << "[EdgeReprojection] WARN: invalid dep_j=" << dep_j
                  << " inv_dep_i=" << inv_dep_i << ", skip residual\n";
        residual_.setZero();
        return;
    }
    ...
    if (!residual_.allFinite()) {
        std::cerr << "[EdgeReprojection] WARN: residual has NaN/Inf, force zero\n";
        residual_.setZero();
    }
...
void EdgeReprojection::ComputeJacobians() {
    ...
    // === 与 ComputeResidual 对齐的合法性检查,异常时把雅可比置零防止污染 H ===
    if (!std::isfinite(inv_dep_i) || inv_dep_i < 1e-3) {
        std::cerr << "[EdgeReprojection] WARN: invalid inv_dep_i=" << inv_dep_i
                  << " in ComputeJacobians, set jacobians zero\n";
        jacobians_[0] = Eigen::Vector2d::Zero();
        jacobians_[1] = Eigen::Matrix<double, 2, 6>::Zero();
        jacobians_[2] = Eigen::Matrix<double, 2, 6>::Zero();
        jacobians_[3] = Eigen::Matrix<double, 2, 6>::Zero();
        return;
    }
