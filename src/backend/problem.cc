...
bool Problem::Solve(int iterations) {
    ...
    while (!stop && (iter < iterations)) {
        ...
        // === NaN 兜底: 当前 chi 已经不是有限值,继续迭代毫无意义,立刻 rollback 退出 ===
        if (!std::isfinite(currentChi_)) {
            std::cerr << "[Solve] FATAL currentChi_ is not finite (" << currentChi_
                      << ") at iter " << iter << ", abort and rollback\n";
            RollbackStates();
            stop = true;
            break;
        }
...
//            RemoveLambdaHessianLM();
            ...
            // === NaN 防护: delta_x 非有限值则不 apply,直接放大 Lambda 重试 ===
            if (!delta_x_.allFinite()) {
                std::cerr << "[Solve] WARN delta_x has NaN/Inf, increase lambda and retry\n";
                currentLambda_ *= ni_;
                ni_ *= 2;
                false_cnt++;
                continue;  // 不要 apply 这次更新
            }
...
//        if (sqrt(currentChi_) < 1e-15)
        // === 修复"假收敛": currentChi_/last_chi_ 含 NaN 时不能视为收敛 ===
        if (std::isfinite(last_chi_) && std::isfinite(currentChi_)
            && last_chi_ - currentChi_ < 1e-5)
        {
...
void Problem::MakeHessian() {
    ...
    // === P0-1: 入口先验自愈 ===
    // 如果上一轮 (Marginalize / UpdateStates) 把先验弄成了 NaN/Inf,
    // 这里直接把它清零,避免污染本轮 b_ 和 H_,系统能继续跑下去。
    if (H_prior_.rows() > 0) {
        if (!H_prior_.allFinite() || !b_prior_.allFinite()) {
            std::cerr << "[MakeHessian] WARN prior contains NaN/Inf, reset prior to zero. "
                      << "H_prior.allFinite=" << H_prior_.allFinite()
                      << " b_prior.allFinite=" << b_prior_.allFinite() << std::endl;
            H_prior_.setZero();
            b_prior_.setZero();
            if (err_prior_.rows() > 0)   err_prior_.setZero();
            if (Jt_prior_inv_.rows() > 0) Jt_prior_inv_.setZero();
            if (b_prior_backup_.rows() > 0)   b_prior_backup_.setZero();
            if (err_prior_backup_.rows() > 0) err_prior_backup_.setZero();
        }
    }
...
//                std::cout << " fixed prior, set the Hprior and bprior part to zero, idx: "<<idx <<" dim: "<<dim<<std::endl;
    ...
    // === NaN/Inf 防护: 在交给求解器之前确认 H 和 b 都是有限值 ===
    // 之前我们用 assert() 直接 abort,这是诊断阶段需要的。
    // 现在已经定位到 NaN 来自 b_prior_,且入口处已自愈先验,
    // 这里改为"打印+重置"软恢复,避免一次坏帧把整个系统打死。
    if (!Hessian_.allFinite() || !b_.allFinite()) {
        std::cerr << "[MakeHessian] FATAL: Hessian/b contains NaN/Inf!"
                  << " H.allFinite=" << Hessian_.allFinite()
                  << " b.allFinite=" << b_.allFinite() << std::endl;
        // 打印第一个出现 NaN 的对角元位置,便于定位是哪个 vertex/edge 引入的
        for (long i = 0; i < Hessian_.rows(); ++i) {
            if (!std::isfinite(Hessian_(i, i)) || !std::isfinite(b_(i))) {
        ...
        // 软恢复: H/b 整个清零,让本轮 SolveLinearSystem 给出 delta_x_=0,
        // 同时清掉先验,等下一帧重新积累。
        Hessian_.setZero();
        b_.setZero();
        if (H_prior_.rows() > 0)  H_prior_.setZero();
        if (b_prior_.rows() > 0)  b_prior_.setZero();
        if (err_prior_.rows() > 0) err_prior_.setZero();
    }
}
...
//        TicToc t_Hmminv;
        ...
        for (auto landmarkVertex : idx_landmark_vertices_) {
            ...
            // 与 Marginalize 保持一致的病态保护:
            //   - inv_dep<=0 / NaN 的 landmark 全部跳过, Hmm_inv 该块保持为 0
            //   - 对角块全零(所有观测被过滤)也跳过
            auto Hmm_block = Hmm.block(idx, idx, size, size);
            if (landmarkVertex.second->TypeInfo() == std::string("VertexInverseDepth")) {
                double inv_dep = landmarkVertex.second->Parameters()[0];
                if (!std::isfinite(inv_dep) || inv_dep <= 0) {
                    continue;
                }
            }
            if (!Hmm_block.allFinite()
                || Hmm_block.diagonal().cwiseAbs().sum() < 1e-12) {
                continue;
            }
            Hmm_inv.block(idx, idx, size, size) = Hmm_block.inverse();
        }
        if (!Hmm_inv.allFinite()) {
            Hmm_inv = (Hmm_inv.array().isFinite()).select(Hmm_inv, 0.0);
        }
        ...
        // === 打印 Hessian 条件数,提前发现接近奇异的情况 ===
        // 仅在 reserve_size 不太大时计算特征分解(避免每次求解都很贵)
        if (reserve_size > 0 && reserve_size <= 200) {
            Eigen::SelfAdjointEigenSolver<MatXX> es(H_pp_schur_);
            if (es.info() == Eigen::Success) {
                double lam_min = es.eigenvalues().minCoeff();
                double lam_max = es.eigenvalues().maxCoeff();
                double cond = (std::abs(lam_min) > 1e-30)
                              ? (lam_max / std::abs(lam_min))
                              : std::numeric_limits<double>::infinity();
                if (!std::isfinite(cond) || cond > 1e12 || lam_min < 1e-8) {
                    std::cerr << "[SolveLinearSystem] WARN H_pp_schur ill-conditioned:"
                              << " lambda_min=" << lam_min
                              << " lambda_max=" << lam_max
                              << " cond=" << cond
                              << " currentLambda=" << currentLambda_ << std::endl;
                }
            } else {
                std::cerr << "[SolveLinearSystem] WARN eigen decomposition failed\n";
            }
        }
        if (!H_pp_schur_.allFinite() || !b_pp_schur_.allFinite()) {
            std::cerr << "[SolveLinearSystem] FATAL H_pp_schur/b_pp_schur has NaN/Inf,"
                      << " skip this solve, force delta_x_=0\n";
            delta_x_.setZero();
            return;
        }
...
//        std::cout << "schur time cost: "<< t_Hmminv.toc()<<std::endl;
    ...
    // === NaN/Inf 兜底: ldlt 在病态时会产生 NaN,绝不能让它流向状态更新 ===
    if (!delta_x_.allFinite()) {
        std::cerr << "[SolveLinearSystem] WARN delta_x has NaN/Inf, force zero\n";
        delta_x_.setZero();
    }
}
...
void Problem::UpdateStates() {
    ...
    // === NaN 防护: delta_x_ 不应该流向状态量 ===
    // 上游 SolveLinearSystem 已经做过一次清零,这里再加一道保险,
    // 防止任何路径下含 NaN 的 delta_x_ 污染 vertex 参数和先验。
    if (!delta_x_.allFinite()) {
        std::cerr << "[UpdateStates] WARN delta_x has NaN/Inf, skip update\n";
        return;
    }
    ...
    if (err_prior_.rows() > 0) {
        ...
        // === P0-2: 立即检验更新后的先验是否仍然合法,出现 NaN 立刻回滚 ===
        // 即便 delta_x_ 是有限值,H_prior_ 自身可能在 marginalization 阶段被坏 landmark 污染过,
        // 一次乘法就足以把 b_prior_ 推到 NaN。这里抓住第一时间 rollback,
        // 避免污染传染到 err_prior_/Jt_prior_inv_ 进而影响后续 chi2 计算。
        if (!b_prior_.allFinite() || !err_prior_.allFinite()) {
            std::cerr << "[UpdateStates] WARN prior became NaN/Inf after update, rollback. "
                      << "b_prior.allFinite=" << b_prior_.allFinite()
                      << " err_prior.allFinite=" << err_prior_.allFinite() << std::endl;
            b_prior_ = b_prior_backup_;
            err_prior_ = err_prior_backup_;
            // 如果 backup 自己也烂了,只能整个清零,等待下一帧重新积累
            if (!b_prior_.allFinite() || !err_prior_.allFinite()) {
                std::cerr << "[UpdateStates] WARN backup also dirty, force prior to zero\n";
                b_prior_.setZero();
                err_prior_.setZero();
            }
        }
...
//    scale += 1e-3;    // make sure it's non-zero :)
    ...
    // === NaN/Inf 防护: tempChi 或 scale 任何一个不是有限值都视作失败步 ===
    if (!std::isfinite(tempChi) || !std::isfinite(scale)) {
        std::cerr << "[IsGoodStepInLM] WARN tempChi=" << tempChi
                  << " scale=" << scale << " not finite, treat as bad step\n";
        currentLambda_ *= ni_;
        ni_ *= 2;
        return false;
    }
    ...
    if (rho > 0 && isfinite(tempChi))   // last step was good, 误差在下降
    {
        ...
        // === Lambda 上下限保护,避免数值爆炸到 inf ===
        currentLambda_ = std::max(currentLambda_, 1e-7);
        currentLambda_ = std::min(currentLambda_, 1e18);
        ni_ = 2;
        currentChi_ = tempChi;
        return true;
    } else {
...
//    std::cout << "pose dim: " << pose_dim <<std::endl;
    ...
    if (marg_landmark_size > 0) {
        ...
        for (auto iter: margLandmark) {
            ...
            // === P1: 对每个 landmark 的 Hmm 对角块做病态检查 ===
            // 三种情况会让该块奇异 / 数值病态:
            //   (a) 该 landmark 的所有观测都被 EdgeReprojection 过滤掉了 (inv_dep<=0/NaN),
            //       Jacobian 全为零 -> Hmm 块为零, .inverse() 直接产生 Inf/NaN。
            //   (b) 单观测 landmark, 三角化没有意义,Hmm 块也接近零。
            //   (c) Parameters()[0] (inv_dep) 本身已经是 NaN/Inf。
            // 对这些 landmark, 我们直接把 Hmm_inv 对应块保留为 0,
            // 等同于"这一行 Schur 互补不贡献信息"。这样 H_prior_ 不会被污染。
            auto Hmm_block = Hmm.block(idx, idx, size, size);
            double diag_abs_sum = Hmm_block.diagonal().cwiseAbs().sum();
            bool block_finite = Hmm_block.allFinite();

            // 顺手检查 inv_dep,出现 inv_dep<=0 给出告警 (主要为追溯前端来源)
            if (iter.second->TypeInfo() == std::string("VertexInverseDepth")) {
                double inv_dep = iter.second->Parameters()[0];
                if (!std::isfinite(inv_dep) || inv_dep <= 0) {
                    std::cerr << "[Marginalize] WARN inverse-depth vertex id="
                              << iter.second->Id() << " has bad inv_dep="
                              << inv_dep << ", skip in Schur complement\n";
                    continue;  // Hmm_inv 该块保持 0,不参与 marg
                }
            }

            if (!block_finite || diag_abs_sum < 1e-12) {
                std::cerr << "[Marginalize] WARN landmark vertex id=" << iter.second->Id()
                          << " has singular Hmm block (finite=" << block_finite
                          << " diag_sum=" << diag_abs_sum
                          << "), skip in Schur complement\n";
                continue;  // 该 landmark 不参与 marg,Hmm_inv 该块保持 0
            }

            Hmm_inv.block(idx, idx, size, size) = Hmm_block.inverse();
        }

        // 最终 NaN 兜底: 即便单块都过了, 累加到 Hmm_inv 后再 check 一次
        if (!Hmm_inv.allFinite()) {
            std::cerr << "[Marginalize] WARN Hmm_inv contains NaN/Inf after block inversion,"
                      << " zero out non-finite entries\n";
            Hmm_inv = (Hmm_inv.array().isFinite()).select(Hmm_inv, 0.0);
        }
